...
def get_report_threat_graph(report_id: str):
    ...
    sub_nodes = [n for n in topology["nodes"] if n.get("id") in associated_node_ids or n.get("type") == "ACTOR"]
    sub_links = [l for l in topology["links"] if l.get("source") in associated_node_ids or l.get("target") in associated_node_ids]
...
# Additional top-level direct aliases as requested in API specifications
direct_router = APIRouter(tags=["Threat Intelligence Direct Endpoints"])

@direct_router.get("/threat-intelligence/{report_id}")
def direct_threat_intel(report_id: str):
    return get_report_threat_intelligence(report_id)

@direct_router.get("/threat-graph/{report_id}")
def direct_threat_graph(report_id: str):
    return get_report_threat_graph(report_id)

@direct_router.get("/risk-score/{report_id}")
def direct_risk_score(report_id: str):
    return get_report_risk_score(report_id)
